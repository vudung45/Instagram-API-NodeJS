...
class InstagramAuth {
    constructor(username, password) {
        ...
        var seed = crypto.createHash('md5').update(utf8.encode(username)).update(utf8.encode(password)).digest('hex')
        ...
        this.sendRequest('si/fetch_headers/?challenge_type=signup&guid=' + this.genUUID(false), null,
            (data) => {
                if (data != null) {
                    ...
                    this.sendRequest('accounts/login/', this.generateSignature(JSON.stringify(send_data)),
                        (data) => {
                            if (data) {
                                try {
                                    ...
                                    var cookie = this.parseCookie(this.cookieJar.getCookieString(API_URL))
    ...
    logout(callback) {
        this.sendRequest('accounts/logout/', null,
            (data) => {
                console.log(data)
                callback(data)
    ...
    getFollowings(maxid = null, callback) {
        if (this.isLoggedIn) {
            ...
            this.sendRequest(endpoint, null,
                (data) =>
                callback(data)
            )
        } else
    ...
    unfollow(userId, callback) {
        ...
        return this.sendRequest('friendships/destroy/' + userId + '/', this.generateSignature(data),
            (data) => {
    ...
    parseCookie(cookieString) {
        // console.log(cookieString) --> key1=value1; key2=value2; ...
    ...
    generateSignature(data, skip_quote = false) {
        var parsedData
        if (!skip_quote)
